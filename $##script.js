...
function countVipStats(a){
    ...
    a.forEach(m=>{
        ...
        if(c>=1&&c<=9){
            b[c]++;
            vipLists[c].push(m);
            // Cek jika bergabung dalam 24 jam terakhir
            if((now - new Date(m.joinDate)) < oneDayInMs) alertStatus[c] = true;
        }
    });
...
window.openVipModal = function(level) {
    ...
    // Urutkan List dari Tanggal Terakhir ke Awal (Terbaru ke Terlama)
    let sortedList = [...vipLists[level]].sort((a,b) => new Date(b.joinDate) - new Date(a.joinDate));

    if (sortedList.length > 0) {
        sortedList.forEach(m => {
            const isNew24h = (now - new Date(m.joinDate)) < oneDayInMs;
            const blinkClass = isNew24h ? 'new-name-alert' : '';
            body.innerHTML += `<div class="v-item ${blinkClass}"><span class="v-n">${m.name} ${isNew24h ? '🆕' : ''}</span><span class="v-u">${m.uid}</span></div>`;
        });
    } else {
