...
function countVipStats(a){
    ...
    let hasNewAchiever = {1:false,2:false,3:false,4:false,5:false,6:false,7:false,8:false,9:false};
    vipLists = {1:[], 2:[], 3:[], 4:[], 5:[], 6:[], 7:[], 8:[], 9:[]};
    
    // Tentukan awal periode aktif untuk deteksi "Baru"
    const now = new Date();
    ...
    a.forEach(m=>{
        ...
        if(c>=1&&c<=9){
            b[c]++;
            vipLists[c].push(m);
            // Jika join di periode ini, nyalakan alert di dashboard
            if(new Date(m.joinDate) >= periodStart) hasNewAchiever[c] = true;
        }
    });
    ...
    for(let i=1;i<=9;i++){
        ...
        if(el){
            ...
            if(hasNewAchiever[i]) parent.classList.add('new-alert');
...
window.openVipModal = function(level) {
    ...
    if (list && list.length > 0) {
        list.forEach(m => {
            const isNew = new Date(m.joinDate) >= periodStart;
            const blinkClass = isNew ? 'new-name-alert' : '';
            body.innerHTML += `<div class="v-item ${blinkClass}"><span class="v-n">${m.name} ${isNew ? '🔥' : ''}</span><span class="v-u">${m.uid}</span></div>`;
        });
    } else {
