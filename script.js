...
// Cek apakah library Supabase berhasil loading
let db = null;
if (window.supabase) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Redirect logic
    if (!isLogged && !path.includes('index.html')) {
    ...
    if (path.includes('index.html')) {
        ...
        // Cek koneksi database saat buka halaman
        if (!db) {
            document.getElementById('error').innerText = "⚠️ Gagal memuat sistem. Cek koneksi internet & Refresh.";
        }
    } 
    // Halaman dalam
    else if (isLogged) {
        await loadData(); // Load data dulu baru render
...
async function loadData() {
    if (!db) return; // Stop jika DB error
    try {
        ...
        console.error("Error Load Data:", a);
        if(window.location.pathname.includes('index.html')) {
            document.getElementById('error').innerText = "Gagal ambil data: " + a.message;
        }
    }
}
...
async function doLogin() {
    ...
    if (!uidInput) return void(err.innerText = "Masukkan UID");
    if (!db) return void(err.innerText = "Sistem Error: Database tidak terhubung.");

    btn.innerText = "Memuat...";
    btn.disabled = true;
    err.innerText = "";

    // Paksa load data terbaru saat tombol ditekan
    await loadData();

    if (globalData.length === 0) {
        err.innerText = "Koneksi Bermasalah. Data Kosong.";
        btn.innerText = "MASUK";
        btn.disabled = false;
        return;
    }
...
function getRankLevel(a){
    ...
    for(const tier of rankTiers) {
        if (totalMembers >= tier.min) {
            let currentVips = 0;
            if (tier.level >= 3) { currentVips = vip2InTeam; } else if (tier.level === 2) { currentVips = vip1InTeam; }
...
function countVipStats(a){
    ...
    let savedVipState = JSON.parse(localStorage.getItem('savedVipState')) || {};
    a.forEach(a=>{
    ...
    for(let i=1; i<=9; i++){
        const cElement = document.getElementById(`cVIP${i}`);
        const btnElement = document.getElementById(`btnVip${i}`);
        if(cElement) cElement.innerText = b[i];
        const currentUids = vipLists[i].map(m => m.uid);
        const savedUids = savedVipState[i] || [];
        const newAchievers = currentUids.filter(uid => !savedUids.includes(uid));
        if (newAchievers.length > 0) {
            newVipAlerts[i] = newAchievers;
            if(btnElement) btnElement.classList.add('blink-alert');
        } else {
            if(btnElement) btnElement.classList.remove('blink-alert');
        }
    }
}
...
window.openVipModal = function(level) {
    ...
    const btnElement = document.getElementById(`btnVip${level}`);
    title.innerText = `DAFTAR V.I.P ${level}`;
    body.innerHTML = ''; 
    const list = vipLists[level];
    const newAlerts = newVipAlerts[level] || []; 
    if (list && list.length > 0) {
        list.forEach(m => {
            const isNew = newAlerts.includes(m.uid);
            const rowClass = isNew ? 'v-item blink-row' : 'v-item';
            body.innerHTML += `<div class="${rowClass}"><span class="v-n">${m.name}</span><span class="v-u">${m.uid}</span></div>`;
        });
    } else {
    ...
    if(btnElement) btnElement.classList.remove('blink-alert');
    let savedVipState = JSON.parse(localStorage.getItem('savedVipState')) || {};
    savedVipState[level] = vipLists[level].map(m => m.uid);
    localStorage.setItem('savedVipState', JSON.stringify(savedVipState));
    newVipAlerts[level] = [];
}
...
function calculateMyRank(currentTeamSize, directDownlineCount, uid) {
    ...
    const vip2InTeam = countSpecificVipInTeam(teamMembers, 2);
    const vip1InTeam = countSpecificVipInTeam(teamMembers, 1);
    let nextGapValue = 0; let nextGoalText = ""; let descHtml = ""; let structureGapText = null; 
    let nextRank = rankTiers.find(r => r.level === currentRankLevel + 1);

    if (currentRankLevel === 9) {
        structureGapText = "Top Level";
    } else if (nextRank) {
        let requiredVips = nextRank.reqVip; let currentVips = 0; let reqLevelName = nextRank.reqLevelName;
        if (nextRank.level >= 3) { currentVips = vip2InTeam; } else if (nextRank.level === 2) { currentVips = vip1InTeam; }
        ...
        } else if (currentRankLevel === 0) { structureGapText = `Direct Downline lagi`; } else { structureGapText = `Menuju ${nextRank.name}`; }
    } else { structureGapText = currentRankLevel === 0 ? "Direct Downline lagi" : "Memuat..."; }
    
    if (currentRankLevel === 9) {
        nextGapValue = 0; nextGoalText = "Menuju Level Maksimal"; descHtml = "Maksimal";
    } else if (nextRank) {
        if (nextRank.level === 1) {
             nextGapValue = nextRank.min - directDownlineCount; nextGoalText = "Menuju V.I.P 1";
             descHtml = `Direct Downline lagi<br><span style="color:#D4AF37;font-weight:bold;">Menuju V.I.P 1</span>`;
        } else {
            let requiredTeamSize = nextRank.min; nextGapValue = requiredTeamSize - currentTeamSize;
        ...
        let nextRankVip1 = rankTiers.find(r => r.level === 1); nextGapValue = nextRankVip1.min - directDownlineCount;
        nextGoalText = "Menuju V.I.P 1"; descHtml = `Direct Downline lagi<br><span style="color:#D4AF37;font-weight:bold;">Menuju V.I.P 1</span>`;
    }
    ...
    if (structureGapText && structureGapText.includes("Tambahan")) { document.getElementById('rankNextGoal').style.color = '#ff4444'; } else { document.getElementById('rankNextGoal').style.color = '#ccc'; }
    document.getElementById('nextLevelGap').innerText = Math.max(0, nextGapValue); 
    const descElement = document.querySelector('.next-desc'); if (descElement) descElement.innerHTML = descHtml;
}
...
// --- FITUR: PERAIH 50% PERIODE LALU & DOWNLOAD TXT ---
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        if (d <= 15) {
            ...
            startP = new Date(prevY, prevM, 16); endP = new Date(prevY, prevM + 1, 0, 23, 59, 59);
            cutoffBase = new Date(prevY, prevM, 16); periodLabel = `PERIODE 2 (${getMonthName(prevM)} ${prevY})`;
        } else {
            startP = new Date(y, m, 1); endP = new Date(y, m, 15, 23, 59, 59);
            cutoffBase = new Date(y, m, 1); periodLabel = `PERIODE 1 (${getMonthName(m)} ${y})`;
        }
            ...
            achievers.forEach((a, index) => {
                ...
                <div class="achiever-item">
                    <div class="achiever-top">
                        <span class="v-n">${index+1}. ${a.name} <span style="color:#D4AF37; font-size:9px;">(VIP ${a.rank})</span></span>
