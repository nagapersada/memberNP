...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Jangan load data otomatis di halaman login untuk menghemat resource
    // Data baru akan diload saat tombol ditekan
    if (b) await loadData();
...
async function loadData() {
    try {
        ...
        console.error(a);
        // Tampilkan pesan error jika terjadi kegagalan load data
        if(window.location.pathname.includes('index.html')) {
            alert("Gagal terhubung ke Database: " + (a.message || "Periksa koneksi internet Anda"));
        }
    }
}
...
async function doLogin() {
    ...
    // Load data fresh dari database setiap kali login
    await loadData();

    // Cek apakah data berhasil diambil
    if (globalData.length === 0) {
        err.innerText = "Gagal mengambil data. Cek koneksi internet.";
        btn.innerText = "MASUK";
        btn.disabled = false;
        return;
    }
...
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(member => {
            ...
            // PERUBAHAN: MENGGUNAKAN Math.floor()
