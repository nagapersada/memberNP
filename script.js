...
// Simpan Data VIP Baru (untuk notifikasi/highlight)
let newVipAchievers = {1:[], 2:[], 3:[], 4:[], 5:[], 6:[], 7:[], 8:[], 9:[]};
...
function renderDashboard(){
    // Cek izin notifikasi saat dashboard dimuat
...
// MODIFIKASI: Menambahkan Logika Pengecekan Anggota Baru
function countVipStats(a){
    ...
    for(let i=1; i<=9; i++){
        ...
        if (newMemberUids.length > 0) {
            ...
            // Tambah Efek Kedip di Dashboard
            if(boxElem) boxElem.classList.add('vip-alert');

            // Trigger Notifikasi (Hanya sekali)
            if (!hasNotification) {
    ...
    // Jika LocalStorage kosong (pertama kali), simpan langsung tanpa alert
...
// MODIFIKASI: Menambahkan Efek Kedip di Modal & Update Storage
window.openVipModal = function(level) {
    const modal = document.getElementById('vipModal');
    const body = document.getElementById('modalBody');
    const title = document.getElementById('modalTitle');
    
    // Hapus alert visual di grid (karena sudah dibuka)
    const gridItems = document.querySelectorAll('.vip-item');
    gridItems.forEach(item => {
        if(item.getAttribute('onclick') === `openVipModal(${level})`) {
            item.classList.remove('vip-alert');
        }
    });
    ...
    if (list && list.length > 0) {
        list.forEach(m => {
            ...
            // Jika member baru, kasih style kedip
    ...
    // Update penyimpanan (tanda sudah dilihat)
    updateVipLocalStorage();
}
