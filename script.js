...
function openAchieverModal() {
    ...
    setTimeout(() => {
        ...
        myTeamData.forEach(member => {
            ...
            // PERUBAHAN DISINI: MENGGUNAKAN Math.floor() SESUAI PERMINTAAN "PEMBULATAN KE BAWAH"
        ...
        if (achievers.length === 0) {
            ...
            achievers.forEach((a, index) => {
                ...
                // Tambahkan ke Text File (Format Telegram)
...
function downloadAchieverData() {
    ...
    const blob = new window.Blob([achieverTxtContent], { type: 'text/plain' });
    const anchor = document.createElement('a');
    anchor.href = window.URL.createObjectURL(blob);
    anchor.download = 'peraih_50_persen.txt';
