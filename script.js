...
// FUNGSI KHUSUS UNTUK CEK RANK V.I.P 1 & V.I.P 2 SAJA (TANPA REKURSI)
function getRankLevel_NoRecur(a){
    const b=getTotalGroupCount(a),c=globalData.filter(b=>b.upline===a).length;
    if (b >= 31) return 2; // VIP 2
    if (c >= 5) return 1; // VIP 1
    return 0;
}

// FUNGSI UTAMA UNTUK CEK RANK DENGAN SYARAT TAMBAHAN
function getRankLevel(a){
    const b=getTotalGroupCount(a),c=globalData.filter(b=>b.upline===a).length;
    
    // Hitung berapa direct downline (langsung) yang sudah V.I.P 2
    let vip2Downlines = 0;
    const directDownlines = globalData.filter(member => member.upline === a);
    directDownlines.forEach(member => {
        if (getRankLevel_NoRecur(member.uid) >= 2) {
            vip2Downlines++;
        }
    });
    // Menambahkan Syarat V.I.P 2 Downlines
    if (b >= 3501 && vip2Downlines >= 2) return 9;
    if (b >= 1601 && vip2Downlines >= 2) return 8;
    if (b >= 901 && vip2Downlines >= 2) return 7;
    if (b >= 501 && vip2Downlines >= 2) return 6;
    if (b >= 351 && vip2Downlines >= 2) return 5;
    if (b >= 201 && vip2Downlines >= 2) return 4;
    if (b >= 101 && vip2Downlines >= 2) return 3;
    if (b >= 31) return 2;
    if (c >= 5) return 1;
    return 0;
}
...
function calculateMyRank(a, b, uid) {
    ...
    // Cek jumlah V.I.P 2 di downline langsung
    let vip2Downlines = 0;
    const directDownlines = globalData.filter(member => member.upline === uid);
    directDownlines.forEach(member => {
        if (getRankLevel_NoRecur(member.uid) >= 2) {
            vip2Downlines++;
        }
    ...
    if (d) {
        ...
        // Cek syarat tambahan 2x V.I.P 2 untuk V.I.P 3 ke atas
        if (e === "V.I.P 2") {
            // Target berikutnya: V.I.P 3
            if (vip2Downlines < 2) {
                h = 2 - vip2Downlines;
                g = `Tambahan ${h} V.I.P 2 di Direct Downline`;
                descHtml = `Downline V.I.P 2 lagi<br><span style="color:#D4AF37;font-weight:bold;">Menuju V.I.P 3</span>`;
                ...
                return; // Keluar dari fungsi jika syarat V.I.P 3 belum terpenuhi
            } else {
                // Syarat VIP 2 sudah terpenuhi, lanjutkan ke syarat jumlah anggota (next rank)
                let nextRank = c[f - 1];
                h = nextRank.min - a;
            ...
            // Rank V.I.P 3 ke atas
            if (vip2Downlines < 2) {
                // Jika sudah mencapai jumlah total, tapi belum punya 2 V.I.P 2,
                // maka rank tertinggi yang bisa dicapai adalah V.I.P 2 (atau rank yang sudah tercapai).
                h = 2 - vip2Downlines;
                g = `Tambahan ${h} V.I.P 2 di Direct Downline`;
                descHtml = `Downline V.I.P 2 lagi<br><span style="color:#D4AF37;font-weight:bold;">Menuju ${e}</span>`;
            } else {
                // Syarat VIP 2 sudah terpenuhi, cek rank berikutnya
                let nextRank = c[f - 1];
                h = nextRank.min - a;
                g = `Menuju ${nextRank.name}`;
                descHtml = `Anggota lagi<br><span style="color:#D4AF37;font-weight:bold;">${g}</span>`;
    ...
    // Penyesuaian: Jika jumlah anggota sudah memenuhi syarat V.I.P X, tapi syarat V.I.P 2 belum, 
    // rank yang ditampilkan harus yang sudah tercapai.
    const currentRank = getRankLevel(uid);
    if (currentRank > 0 && e !== c[currentRank-1]?.name && e !== "MEMBER") {
        e = currentRank === 1 ? "V.I.P 1" : "V.I.P 2";
        if (currentRank === 2) {
             h = 2 - vip2Downlines;
             g = `Tambahan ${h} V.I.P 2 di Direct Downline`;
             descHtml = `Downline V.I.P 2 lagi<br><span style="color:#D4AF37;font-weight:bold;">Menuju V.I.P 3</span>`;
    ...
    } else if (currentRank === 0 && e !== "MEMBER") {
        e = "MEMBER";
        h = 5 - b;
        g = "Menuju V.I.P 1";
        descHtml = `Direct Downline lagi<br><span style="color:#D4AF37;font-weight:bold;">Menuju V.I.P 1</span>`;
