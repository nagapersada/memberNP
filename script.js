...
// Cek apakah Supabase berhasil dimuat
let db = null;
if (window.supabase) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (path.includes('index.html')) {
        ...
        // Cek koneksi awal
        if (!db) {
            document.getElementById('error').innerText = "⚠️ Gagal memuat sistem Database. Coba Refresh.";
        }
    } 
    else if (isLogged) {
        await loadData();
...
async function doLogin() {
    ...
    try {
        ...
        if (!globalData || globalData.length === 0) {
            throw new Error("Data kosong atau koneksi bermasalah.");
        }
        ...
        if (user) {
            ...
            alert("UID " + uidInput + " tidak ditemukan di database.");
            btn.innerText = "MASUK";
            btn.disabled = false;
        }
    } catch (error) {
